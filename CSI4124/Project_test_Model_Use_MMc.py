...
k_servers = 6      # server number
simulation_time = 3.0 # simulation Time
...
#        continue
#    print(f"Client {i.customer_id}:")
#    print(f"  Arrival Time     : {i.arrival_time:.2f}")
#    print(f"  InterArrival Time     : {i.interArrival_time:.2f}")
 #   print(f"  Service Start    : {i.start_service:.2f}")
  #  print(f"  Service Time     : {i.service_time:.2f}")
   # print(f"  Waiting Time     : {i.waiting_time:.2f}")
    #print(f"  Departure Time   : {i.departure_time:.2f}")
 #   print(f"  Time in System   : {i.time_in_system:.2f}")
  #  print(f"  Server ID        : {i.server_id}")
   # print("-" * 40)
#load data into a DataFrame object:
data = {
    "Customer_id" : [ customer.customer_id for customer in customers if customer.departure_time != None],
    "Arrival": [ customer.arrival_time for customer in customers if customer.departure_time != None], #create a list of arrival time
